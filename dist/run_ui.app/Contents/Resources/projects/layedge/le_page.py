...
class LePage(BasePage):
    ...
    def verify_single_button(self, ele):
        ...
        while attempt < max_attempts:  # 限制尝试次数
            try:
                attempt += 1
                self.logger.info(f"开始第 {attempt}/{max_attempts} 次验证尝试")
                result = self._verify(ele)
                
                # 验证失败的情况
                if len(result) >= 3:
                    self.logger.warning(f"第 {attempt} 次验证未通过")
                    # 处理验证失败，等待60秒
                ...
                self.logger.error(f"第 {attempt} 次验证过程发生错误: {str(e)}")
                self.wait_s(2)
        
        self.logger.error(f"验证失败，已达到最大尝试次数 {max_attempts}")
        return False
